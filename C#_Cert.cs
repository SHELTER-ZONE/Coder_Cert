...
public class Program
{
    ...
    public class LockSolver
    {
        ...
        public void Run(Func<string, string> function)
        {
            ...
            if (CheckType(typeof(string), sampleTest).ToString() != "2353")
                throw new Exception($"Your Solve() method returned '{sampleTest}' instead of '2353'. Please " +
                                    "double check whether your logic is correct.");
            ...
            foreach (KeyValuePair<string, string> keyValuePair in TestCaseDictionary)
            {
                ...
                if (TestCaseDictionary[keyValuePair.Key] != null && functionResult != TestCaseDictionary[keyValuePair.Key])
                {
        ...
        public string GetPassword(string userID)
        {
            ...
            foreach (string ans in TestAnswers)
            {
                builder.Append(ans);
            }
            string answer = builder + $"{Function(CheckType(typeof(string), userID).ToString())}";
    ...
    static string Solve(string token)
    {
        ...
        // You need C# 8.0 to have this script running without errors.
        // You also need to install System.Text.Json from NuGet to have this script running without errors.
    }
