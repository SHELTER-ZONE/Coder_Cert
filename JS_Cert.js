...
const LockSolver = () => {
  ...
  const getHash= (M,N=!1,T=1337) => {
    var D,z,O=void 0===T?2166136261:T;
    
    for(D=0,z=M.length;D<z;D++) O^=M.charCodeAt(D),O+=(O<<1)+(O<<4)+(O<<7)+(O<<8)+(O<<24);
  ...
  const __run = (callback) => {
    let sampleAns = callback('12345');
  ...
  const __get = (userID) => {
    ...
    answer = userAns.join('') + callbackFunc(userID);
